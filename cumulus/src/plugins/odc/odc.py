...
from .FFTravelDatabase import FFTravelDatabase
from .ODCWalker import ODCWalker, isHierarchical


def ODCselector(cell, top=None):
    first = True
    help = """Commandes:
    ...
    while True:
        ...
        if sel == "exit":
            ...
            if len(sel) <= len("open") + 1:
                ...
                continue
            try:
                if ODCselector(cells[int(sel[len("open") + 1 :])], top):
                    return True
            except IndexError:
...
class odc:
    ...
    def select(self, cell: Cell, force_simplify=False, refresh_rate=2):
        ...
        travel_db = FFTravelDatabase(self)
        runner = Thread(target=self.run_travel, args=(cell, travel_db))
        runner.start()
        ...
        while not self._done.is_set():
            ...
            self.printv(
                ODCVerbose.Normal,
                f"Selected ff : {len(travel_db)} ffs",
            )
        ...
        self.printv(ODCVerbose.Mini, f"  Results: {len(travel_db)} flip-flops")
        self.printv(ODCVerbose.Mini, "Selection done.")
        self._selection |= travel_db._ffs
        ODCWalker.walker_number = 0
        ODCWalker.iter_count = 0
        ODCWalker.iter_rep = list()
